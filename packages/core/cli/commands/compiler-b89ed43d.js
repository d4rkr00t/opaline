...
async function parseCommands(project, commands) {
    return await Promise.all(commands.map(command => parseCommand(project, command)));
}
...
function getMetaFromJSDoc({ jsdocComment, cliName }) {
    ...
    return {
        ...
        usage: (jsdoc.tags.find(tag => tag.title === "usage") || { description: "" }).description.replace("{cliName}", cliName),
        examples: jsdoc.tags
            .filter(tag => tag.title === "example")
            .map(tag => tag.description.replace("{cliName}", cliName)),
        shouldPassInputs: !!jsdoc.tags.find(tag => tag.title === "param" && tag.name === "$inputs"),
        options: jsdoc.tags.reduce((acc, tag) => {
            if (tag.title !== "param" || tag.name === "$inputs")
                return acc;
            acc[tag.name] = {
...
function createEntryPoint({ project, commandsData }) {
    let pkgJsonRelativePath = path.relative(path.dirname(project.binOutputPath), project.pkgJson.path);
...
let config = {
  ...
  commands: {
    ${commandsData
      ...
      load: () => require("${getRelativeCommandPath(project.binOutputPath, command.commandName)}")
    }`)
        .join(", ")}
  }
};
...
function getRelativeCommandPath(binOutputPath, commandName) {
    return ("." +
        path.sep +
        path.relative(binOutputPath, path.join(binOutputPath, "commands", commandName)));
}
...
class Compiler {
    ...
    async getCommands() {
        return (await readdir(this.project.commandsDirPath)).filter(file => !file.endsWith(".d.ts") &&
            !file.endsWith(".map") &&
            !file.startsWith("_"));
    }
    ...
    async build() {
        ...
        try {
            ...
            let message = [
                chalk.green(`🦄 Built in ${(endTime[0] / 1000 + endTime[1] / 1e6).toFixed(2)}ms!`),
                ""
            ];
            let outputPath = this.project.commandsOutputPath.replace(this.project.projectRootDir + path.sep, "");
            for (let bundle of output.output) {
                if (bundle.type === "chunk" && bundle.isEntry) {
                    message.push(`${chalk.grey("– " + outputPath + path.sep)}${chalk.greenBright(bundle.fileName)}`);
                }
            }
            core.print(message);
        }
    ...
    async watch() {
        ...
        watcher
            .on("add", async (file) => {
            // print(chalk.grey("New command has been added: ") + file);
            await this.updateCommands();
            this.createWatchBundler();
        })
            .on("unlink", async (file) => {
            // print(chalk.grey("Command has been deleted: ") + file);
            await this.updateCommands();
            this.createWatchBundler();
        });
    }
    async createWatchBundler() {
        let relativePathToCommands = this.project.commandsDirPath.replace(this.project.projectRootDir + path.sep, "") + path.sep;
        if (this.watcher) {
            this.watcher.close();
            core.print([
                "",
                ...
                "",
                ...this.commands.map(c => chalk.grey("– " + relativePathToCommands) + chalk.greenBright(c))
            ]);
        }
        else {
